    ...
    methods
        function obj = Plotting(style,downdip,evl,xlocs,gravity,G33)
            ...
            if ~exist(strcat(evl.knlpath,'KO_s.grd'),'file')
                ...
                obj.KO={Ks,Kd};
                grdwrite([0 1], [0 1],Ks,[evl.knlpath,'KO_s.grd'])
                grdwrite([0 1], [0 1],Kd,[evl.knlpath,'KO_d.grd'])
            else
                [~,~,Ks]=grdread([evl.knlpath,'KO_s.grd']);
                [~,~,Kd]=grdread([evl.knlpath,'KO_d.grd']);
                obj.KO={Ks,Kd};
            end
            if evl.shz.N > 0
                if ~exist(strcat(evl.knlpath,'LO_11.grd'),'file')
                    ...
                    grdwrite([0 1], [0 1],L11,[evl.knlpath,'LO_11.grd'])
                    grdwrite([0 1], [0 1],L12,[evl.knlpath,'LO_12.grd'])
                    grdwrite([0 1], [0 1],L13,[evl.knlpath,'LO_13.grd'])
                    grdwrite([0 1], [0 1],L22,[evl.knlpath,'LO_22.grd'])
                    grdwrite([0 1], [0 1],L23,[evl.knlpath,'LO_23.grd'])
                    grdwrite([0 1], [0 1],L33,[evl.knlpath,'LO_33.grd'])
                else
                    [~,~,L11]=grdread([evl.knlpath,'LO_11.grd']);
                    [~,~,L12]=grdread([evl.knlpath,'LO_12.grd']);
                    [~,~,L13]=grdread([evl.knlpath,'LO_13.grd']);
                    [~,~,L22]=grdread([evl.knlpath,'LO_22.grd']);
                    [~,~,L23]=grdread([evl.knlpath,'LO_23.grd']);
                    [~,~,L33]=grdread([evl.knlpath,'LO_33.grd']);
                    obj.LO={L11,L12,L13,L22,L23,L33};
                end
            else
                obj.LO={0,0,0,0,0,0};
