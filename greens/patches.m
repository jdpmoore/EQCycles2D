    ...
    methods(Static)
        ...
        function [varargout] = seg2flt(filename)
            ...
            switch nColumn
                ...
                case 13 % no Vpl
                    [~,x1,x2,x3,len,width,str,d,rak,lo,wo,alphal,alphaw]=...
                        ...
                        'commentstyle','shell');
                otherwise
                    error('patch:invalid file format');
            end
            ...
            for k=1:length(x1)
                ...
                flt=flt2flt([x1(k);x2(k);x3(k)],len(k),width(k),...
                    str(k),d(k),rak(k),...
                ...
                switch nColumn
                    ...
                    otherwise
                        error('geometry:patch:coding error');
                end
                % segment
                seg{k}=segment(x2(k),x1(k),-x3(k),len(k),width(k),str(k),d(k),rak(k),size(fm,1),size(flt,1));
