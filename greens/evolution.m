classdef evolution < handle
    ...
    methods
        ...
        function o=evolution(src,flt,shz,evts,varargin)
            ...
            o.evt=evolution.eventKernels(evts,flt,shz,o.knlpath);
        end
                        ...
                        if ~exist(fnameKK{1},'file')
                            if ~exist(fnameKKmat,'file')
                                ...
                                for i = 1:numel(evt{k}.KK)
                                    grdwrite([0 1], [0 1],evt{k}.KK{i},fnameKK{i})
                                end
                            else
                                load(fnameKKmat);
                                evt{k}.KK=sKK;
                        ...
                        if ~exist(fnameKL{1},'file')
                            if ~exist(fnameKLmat,'file')
                                ...
                                for i = 1:numel(evt{k}.KL)
                                    grdwrite([0 1], [0 1],evt{k}.KL{i},fnameKL{i})
                                end
                            else
                                load(fnameKLmat);
                                evt{k}.KL=sKL;
