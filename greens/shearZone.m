classdef shearZone < handle
    ...
    methods
        ...
        function plotShearZoneEdges(obj,varargin)
            ...
            if 1>obj.N
                fprintf('shearzone: nothing to plot\n');
                return
            end
            ...
            if 1>=nargin
                ...
                for k=1:2
                    for j=1:2
                        ...
                        plot3(x1(:),x2(:),x3(:),'k-','LineWidth',0.01);
                    end
                end
                ...
                [xp,yp,zp,up]=transform4patch_general(...
                ...
                [xp,yp,zp,up]=transform4patch_general(...
                  ...
                  -(obj.x(:,3)+obj.sv(:,3).*obj.L/2-obj.nv(:,3).*obj.T/2), ...
                    varargin{1},...
                    obj.W(:),obj.T(:),obj.dip(:),obj.strike(:)+90);
                 patch(xp,yp,-zp,up,'EdgeColor','None');
        ...
        function exportSideXYZ(o,sideIndex,scale,fname,field)
            ...
            for k=1:3
                switch sideIndex
                    ...
                    otherwise
                        error('shearZone.exportTopSideXYZ: invalid side index.');
                end
            end
            [xp,yp,zp]=deal(xyz{:});
    ...
    methods(Static)
        ...
        function [varargout] = lvl2shz(filename)
            ...
            for k=1:length(x1)
                ...
                % level
                lvl{k}=level(x2(k),x1(k),-x3(k),len(k),width(k),thick(k),str(k),d(k),size(fm,1),size(shz,1));
